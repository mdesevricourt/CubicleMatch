...
import cubiclematch.agent as agent
import cubiclematch.cubicle as cubicle
from cubiclematch.alter import SmallMarket
from cubiclematch.market import Market
...
def cubicle_builder(number_of_cubicles, number_of_half_days):
    ...
    cubicles = [
        cubicle.Cubicle("C" + str(i), number_of_half_days)
        for i in range(1, number_of_cubicles + 1)
    ]
...
if alternative_algorithm:
    ...
    # set price for last cubicle
    small_market.cubicles[-1].prices = np.array(
        [
            22.21065034,
            20.85584706,
            41.25183526,
    ...
    small_market.assign_halfdays(N=1)
    # export allocation
