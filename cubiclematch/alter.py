...
class SmallMarket:
    ...
    def assign_cubicles(self, byyear=False, verbose=False):
        ...
        for agent in agents_by_budget:
            ...
            least_agents = min(
                [len(cubicle.assigned_agents) for cubicle in cubicle_list]
            )
            least_agents_cubicles = [
                cubicle
                for cubicle in cubicle_list
                if len(cubicle.assigned_agents) == least_agents
            ]
            ...
            min_U = [
                np.min(
                    [
                        np.linalg.norm(agent.U - agent2.U)
                        for agent2 in cubicle.assigned_agents
                    ]
                )
                for cubicle in least_agents_cubicles
            ]
    ...
    def assign_halfdays(self, N=100):
        ...
        for cubicle in self.cubicles:
            ...
            market = Market(
                cubicle.assigned_agents, [cubicle], maxing_price=self.maxing_price
            )
            market.find_ACE(N=N, verbose=True)
            market.export_allocation(f"CE_{cubicle}")
            market.pricing_out(verbose=True)
            market.export_allocation(f"pricing_out_{cubicle}")
    ...
    def export_allocation(self, filename):
        ...
        for cubicle in self.cubicles:
            ...
            agents_in_cubicle = [
                agent for agent in self.agents if agent.cubicle == cubicle.number
            ]
